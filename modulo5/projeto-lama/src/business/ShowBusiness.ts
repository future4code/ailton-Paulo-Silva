...
export class ShowBusiness {
  ...
  public createShow = async (
    ...
    if (typeof startsAt !== "string") {
      throw new RequestError(
        "Parâmetro 'startsAt' inválido: deve ser uma string"
      );
    }
    ...
    if (!startsAtDate.getDate()) {
      throw new RequestError(
        "Parâmetro 'startsAt' inválido: deve ser aaaa/mm/dd"
      );
    }
    ...
    if (startsAtDate < festivalStartDate) {
      throw new UnprocessableError(
        "A data do show não pode ser anterior a 2022/12/05"
      );
    }

    const showAlreadyExists = await this.showDatabase.findShowByDate(
      startsAtDate
    );
  ...
  public buyTicket = async (
    ...
    const isTicketAlreadyExists = await this.showDatabase.findTicket(
      showId,
      userId
    );
  ...
  public deleteTicket = async (
    input: IDeleteTicketInputDTO
  ): Promise<IDeleteTicketOutputDTO> => {
    const { token, showId } = input;
    ...
    if (!ticketDB) {
      throw new NotFoundError(
        "Você ainda não comprou um ingresso para esse show"
      );
    }
